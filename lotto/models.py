...
from django.utils import timezone #지정한 특정 시간대 꺼내와줌(settings.py의 TIME_ZONE 영향받)
import random
...
class GuessNumbers(models.Model):
    ...
    def generate(self): # 로또 번호를 자동으로 생성
        ...
        self.save() # GuessNumbers object를 DB에 저장 == conn.commit()

    def __str__(self): # 필요성: Admin page(관리자 페이지)에서 display되는 텍스트에 대한 변경
